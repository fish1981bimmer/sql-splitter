...
- **dbo两段式无schema_prefix时保留不替换(BUG)** — `dbo.xxx`在无schema_prefix时直接去掉dbo变成`xxx`（之前无prefix时保留dbo.不替换，违背用户要求）。有prefix时仍替换为`{prefix}.xxx`。修改位置：`_replace_dbo_prefix()`
...
- **6条用户规则验证通过**：1) []替换规则：`[aa]`→`"aa"`(去[]加双引号)、`[hrbi].[xxx]`→`"hrbi"."xxx"`(schema/表名加双引号)、`[int]`→`int`/`[nvarchar]`→`nvarchar`(类型名去[]不加双引号，后续做类型映射) 2) dbo统一删除(三段式`[HRBI].[dbo].[xxx]`→`"HRBI"."xxx"`/两段式`[dbo].[xxx]`→`"xxx"`/混合式`HRBI_Stage.[dbo].[xxx]`→`HRBI_Stage."xxx"`) 3) VARCHAR(n)加CHAR定义 4) DATETIME/DATETIME2→TIMESTAMP 5) NCHAR→CHAR 6) SELECT INTO #临时表→CTAS创建GTT
- **7条用户规则验证通过**：1) []替换规则：`[aa]`→`"aa"`(去[]加双引号)、`[hrbi].[xxx]`→`"hrbi"."xxx"`(schema/表名加双引号)、`[int]`→`int`/`[nvarchar]`→`nvarchar`(类型名去[]不加双引号，后续做类型映射) 2) dbo统一删除(三段式`[HRBI].[dbo].[xxx]`→`"HRBI"."xxx"`/两段式`[dbo].[xxx]`→`"xxx"`/混合式`HRBI_Stage.[dbo].[xxx]`→`HRBI_Stage."xxx"`) 3) VARCHAR(n)加CHAR定义 4) DATETIME/DATETIME2→TIMESTAMP 5) NCHAR→CHAR 6) SELECT INTO #临时表→CTAS创建GTT 7) 过程体内DDL语句结尾加分号
- **312个存储过程转换全部成功**
...
- **dbo统一删除(v3.5.3起)** - 不再区分三段式/两段式，dbo在任何位置都直接删除
 - 三段式 `[HRBI].[dbo].[Users]` → `"HRBI"."Users"`（删除dbo.）
 - 两段式 `[dbo].[Users]` → `"Users"`（删除dbo.）
 - 混合式 `HRBI_Stage.[dbo].[Users]` → `HRBI_Stage."Users"`（删除dbo.）
...
详见 [产品化规划](references/sql-splitter-productization.md)。
...
> license_checker.py已于v3.5.1移除，社区版对象数量/文件大小限制不再强制执行。大型SQL文件可直接用split_sql_v21.py拆分，无需绕过。
>
> 如仍需无限制拆分，可用unlimited_split.py脚本（绕过旧版MAX_OBJECTS=20限制）：
> ```bash
> python3 ~/.hermes/skills/sql-splitter/scripts/unlimited_split.py input.sql output_dir --dialect sqlserver
> # 只提取存储过程: --type proc
> ```
> 
> ⚠️ unlimited_split.py的`--type proc`过滤功能有BUG：拷贝到filter目录的文件数为0，原因是split内部分两步——先全量拆分到output_dir，再拷贝匹配前缀的文件到filter_dir——当内部路径不一致时拷贝失败。**推荐做法：直接用cp手动提取**：
> ```bash
> python3 ~/.hermes/skills/sql-splitter/scripts/unlimited_split.py input.sql output_dir --dialect sqlserver
> mkdir -p output_dir_proc && cp output_dir/proc_*.sql output_dir_proc/
> ```
...
> ⚠️ license_checker.py已从代码库移除。License管理功能待重新实现。
...
- 详见 [v3.4.5修复记录](references/dm-converter-v345-fixes.md)
