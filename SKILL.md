...
- **6条用户规则验证通过**：1) [和]去掉不加双引号 2) dbo智能处理(三段式去dbo/两段式替换或去掉) 3) VARCHAR(n)加CHAR定义 4) DATETIME/DATETIME2→TIMESTAMP 5) NCHAR→CHAR 6) SELECT INTO #临时表→CTAS创建GTT
- **7条用户规则验证通过**：1) [和]去掉不加双引号 2) dbo智能处理(三段式去dbo/两段式替换或去掉) 3) VARCHAR(n)加CHAR定义 4) DATETIME/DATETIME2→TIMESTAMP 5) NCHAR→CHAR 6) SELECT INTO #临时表→CTAS创建GTT 7) 过程体内DDL语句结尾加分号
