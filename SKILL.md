...
- **6条用户规则验证通过**：1) []替换规则：列名`[aa]`→`aa`(去[]不加双引号)、schema/表名`[hrbi].[xxx]`→`"hrbi"."xxx"`(去[]加双引号)、类型名`[nvarchar]`→`nvarchar`(去[]做类型映射) 2) dbo智能处理(三段式去dbo/两段式替换或去掉) 3) VARCHAR(n)加CHAR定义 4) DATETIME/DATETIME2→TIMESTAMP 5) NCHAR→CHAR 6) SELECT INTO #临时表→CTAS创建GTT
- **7条用户规则验证通过**：1) []替换规则：列名`[aa]`→`aa`(去[]不加双引号)、schema/表名`[hrbi].[xxx]`→`"hrbi"."xxx"`(去[]加双引号)、类型名`[nvarchar]`→`nvarchar`(去[]做类型映射) 2) dbo智能处理(三段式去dbo/两段式替换或去掉) 3) VARCHAR(n)加CHAR定义 4) DATETIME/DATETIME2→TIMESTAMP 5) NCHAR→CHAR 6) SELECT INTO #临时表→CTAS创建GTT 7) 过程体内DDL语句结尾加分号
- **312个存储过程转换全部成功**(HRBI_Stage=54, HRBI_DW=157, HRBI_DM=101, 其中174处SELECT INTO临时表转为CTAS，2057处VARCHAR加CHAR)
...
社区版限制对象数≤20和文件≤1MB。对于超过限制的大文件，可用exec patch方式绕过（不修改原始文件）：
...
# ⚠️ 修复dialect参数：v21函数内部用dialect.value，传字符串会报AttributeError
# 必须在dialect.value调用前加字符串→枚举转换
...
# 使用
result = split_fn(input_file, output_dir, dialect='sqlserver', verbose=True)
```

⚠️ **dialect参数陷阱**：v21的`split_sql_file`函数中`dialect`参数期望`SQLDialect`枚举，传字符串（如`'sqlserver'`）会在第714行`dialect.value.upper()`处触发`AttributeError: 'str' object has no attribute 'value'`。必须先转换为枚举再调用，或在exec patch中加入字符串→枚举适配代码。
