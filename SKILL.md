...
- **6条用户规则验证通过**：1) []替换规则：`[aa]`→`"aa"`(去[]加双引号)、`[hrbi].[xxx]`→`"hrbi"."xxx"`(schema/表名加双引号)、`[int]`→`int`/`[nvarchar]`→`nvarchar`(类型名去[]不加双引号，后续做类型映射) 2) dbo智能处理(三段式去dbo/两段式替换或去掉) 3) VARCHAR(n)加CHAR定义 4) DATETIME/DATETIME2→TIMESTAMP 5) NCHAR→CHAR 6) SELECT INTO #临时表→CTAS创建GTT
- **7条用户规则验证通过**：1) []替换规则：`[aa]`→`"aa"`(去[]加双引号)、`[hrbi].[xxx]`→`"hrbi"."xxx"`(schema/表名加双引号)、`[int]`→`int`/`[nvarchar]`→`nvarchar`(类型名去[]不加双引号，后续做类型映射) 2) dbo智能处理(三段式去dbo/两段式替换或去掉) 3) VARCHAR(n)加CHAR定义 4) DATETIME/DATETIME2→TIMESTAMP 5) NCHAR→CHAR 6) SELECT INTO #临时表→CTAS创建GTT 7) 过程体内DDL语句结尾加分号
- **312个存储过程转换全部成功**
...
> ⚠️ license_checker.py已移除，社区版对象数量/文件大小限制不再强制。大型SQL文件可直接拆分。
