...
- **dbo替换规则(v3.5.4起)** — 三段式和两段式区别处理：
  ...
  - 实现位置：`_replace_dbo_prefix()`，正则按顺序执行: 双点号→三段式引号→三段式裸名→三段式混合→两段式引号→两段式裸名
...
- **dbo替换规则(v3.5.6起)** — 三段式和两段式区别处理:
  - **三段式**（已有schema）: dbo直接删除。`[HRBI_Stage].[dbo].[Users]` → `"HRBI_Stage"."Users"` / `HRBI_Stage.[dbo].[Users]` → `HRBI_Stage."Users"` / `dbo.Users`（在三段式上下文中）→ `Users`
  - **两段式**（无schema）: dbo替换为schema_prefix。`[dbo].[Users]` → `"hrbi_stage"."Users"` / `dbo.Users`（独立两段式）→ `hrbi_stage.Users`
  - **关键区分**: 如果输入中已有其他schema名（如`HRBI_Stage.[dbo].[xxx]`），dbo是冗余默认schema，直接删除；如果输入只有`[dbo].[xxx]`或`dbo.xxx`（无前缀schema），dbo就是唯一的schema标识，替换为schema_prefix
  - 双点号`xxx..yyy` → `xxx.yyy`（SQL Server省略dbo的写法）
  - 实现位置: `_replace_dbo_prefix()`，正则按顺序执行: 双点号→三段式引号→三段式裸名→三段式混合→两段式引号→两段式裸名
 - schema_prefix仅用于标识符的双引号包裹和dbo删除，不再替换dbo为schema前缀
